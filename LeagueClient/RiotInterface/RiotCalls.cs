...
using System.Linq;
using LeagueClient.RiotInterface.Riot.Kudos;
...
namespace LeagueClient.RiotInterface.Riot {
  public class RiotCalls {
    ...
    /// <param name="UUID">The generated UUID of the lobby (random string seems to work)</param>
    /// <returns></returns>
    public static Task<LobbyStatus> CreateGroupFinderLobby(Int32 QueueId, String UUID) {
    ...
    public static SerializationContext RegisterObjects() {
      ...
      var x = from type in LeagueClient.GetTypes()
              where type.IsPublic && (
                string.Equals(type.Namespace, "LeagueClient.RiotInterface.Riot.Platform", StringComparison.Ordinal) ||
                string.Equals(type.Namespace, "LeagueClient.RiotInterface.Riot.Leagues", StringComparison.Ordinal) ||
                string.Equals(type.Namespace, "LeagueClient.RiotInterface.Riot.Team", StringComparison.Ordinal))
              select type;
